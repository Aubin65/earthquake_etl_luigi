...
class MongoCellTarget(MongoTarget):
    ...
    def read(self):
        ...
        result = self.get_collection().aggregate(
            [{"$match": {"_id": self._document_id}}, {"$project": {"_value": "$" + self._path, "_id": False}}]
        )
...
class MongoRangeTarget(MongoTarget):
    ...
    def read(self):
        ...
        cursor = self.get_collection().find(
            {"_id": {"$in": self._document_ids}, self._field: {"$exists": True}}, {self._field: True}
        )
    ...
    def get_empty_ids(self):
        ...
        """
        cursor = self.get_collection().find(
            {"_id": {"$in": self._document_ids}, self._field: {"$exists": True}}, {"_id": True}
        )
